...
class CinemanaApi {
  ...
  Future<List<MediaItem>> search(
    ...
    final q = query.trim();
    final hasAdvancedFilter = category.trim().isNotEmpty || star.trim().isNotEmpty || fromYear != null || toYear != null;
    if (q.isEmpty && !hasAdvancedFilter) return const <MediaItem>[];
    ...
    final merged = _unique(batches.expand((e) => e).toList());
    return q.isEmpty ? merged : _rankSearch(merged, q);
  }
  ...
  Future<dynamic> _advancedSearchExactFast({
    ...
    final params = <String, dynamic>{
      ...
      if (category.trim().isNotEmpty) 'categoryNb': category.trim(),
  ...
  Future<List<int>> availableSearchYears() async {
    try {
      final raw = await _get(
  ...
  Future<List<MediaSection>> collections({bool refresh = false}) async {
    ...
    for (final map in _flattenMaps(raw)) {
      ...
      var items = _mediaList(map['videos'] ?? map['items'] ?? map['videoInfo']);
  ...
  Future<List<MediaItem>> collectionVideos(String collectionId) async {
    ...
    for (final path in <String>[
      ...
      try {
        final raw = await _get(path, ttl: const Duration(minutes: 15), requireNonEmpty: false);
  ...
  double _extractImdbRating(dynamic raw, String requestedId) {
    final cleanId = requestedId.trim();
    ...
    double parse(dynamic value) {
      ...
      if (direct != null && direct > 0 && direct <= 10) return direct;
      final match = RegExp(r'(?<!\\d)(10(?:\\.0+)?|[0-9](?:\\.[0-9]+)?)(?!\\d)').firstMatch(text);
      final parsed = double.tryParse(match?.group(1) ?? '') ?? 0;
      return parsed > 0 && parsed <= 10 ? parsed : 0;
    }

    String objectId(Map map) {
      for (final key in const ['nb', 'videoId', 'videoID', 'id', 'item_id', '_id']) {
    ...
    double ratingFrom(Map map) {
      for (final key in const ['imdbRating', 'imdbRate', 'rate']) {
        if (!map.containsKey(key)) continue;
        final value = parse(map[key]);
        if (value > 0) return value;
      }
      return 0;
    }

    Iterable<Map> maps(dynamic value) sync* {
      if (value is Map) {
        yield value;
        for (final child in value.values) {
          if (child is Map || child is List) yield* maps(child);
        }
    ...
    // First use the object whose id matches the requested Cinemana video id.
    if (cleanId.isNotEmpty) {
      for (final map in maps(raw)) {
        if (objectId(map) != cleanId) continue;
        final value = ratingFrom(map);
        if (value > 0) return value;
      }
    }

    // Then inspect only Cinemana's primary video object when it does not
    // repeat the id. This avoids taking `rate` from recommendations/people.
    if (raw is Map) {
      for (final key in const ['videoInfo', 'video', 'data']) {
        final value = raw[key];
        Map? primary;
        if (value is Map) primary = value;
        if (value is List && value.isNotEmpty && value.first is Map) {
          primary = value.first as Map;
        }
        if (primary == null) continue;
        final id = objectId(primary);
        if (id.isEmpty || cleanId.isEmpty || id == cleanId) {
          final rating = ratingFrom(primary);
  ...
  Future<double> imdbRating(String id, {bool refresh = false}) async {
    ...
    try {
      ...
      return _extractImdbRating(raw, cleanId);
    } catch (_) {
      return 0;
    }
  }
  ...
  Future<ContentDetails> details(String id, {bool refresh = false}) async {
    ...
    // The native Cinemana response can keep imdbRating outside the first
    // videoInfo map. Search the complete response and inject only the real
    // IMDb value into the detail object consumed by the UI.
    final imdb = _extractImdbRating(raw, id);
    final enriched = <String, dynamic>{...map};
    if (imdb > 0) enriched['imdbRating'] = imdb;
    return ContentDetails.fromJson(enriched);
  }
