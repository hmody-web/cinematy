...
class MediaItem {
  ...
  bool get isEpisodeResume =>
      isSeries &&
      (season ?? 0) > 0 &&
      (episode ?? 0) > 0 &&
      _seriesRootId.isNotEmpty;

  String get _seriesRootId {
    for (final key in const <String>[
      ...
      if (value.isNotEmpty && value != '0' && value != id) return value;
    }
    return '';
  }

  /// For a saved episode, return the parent-series item used by the details
  /// screen while keeping this episode's season/episode numbers on the original
  /// snapshot for resume selection.
  MediaItem get seriesRootForResume {
    final rootId = _seriesRootId;
    if (rootId.isEmpty) return this;

    String value(String key, String fallback) {
      final current = raw[key]?.toString().trim() ?? '';
      return current.isEmpty ? fallback : current;
    }

    final rootRaw = Map<String, dynamic>.from(raw)
      ..['nb'] = rootId
      ..['rootSeries'] = rootId
      ..['isSeries'] = true;
    ...
    return MediaItem(
      id: rootId,
      title: value('_seriesTitle', title),
      description: value('_seriesDescription', description),
      posterUrl: normalizeMediaUrl(value('_seriesPoster', posterUrl)),
      backdropUrl: normalizeMediaUrl(value('_seriesBackdrop', backdropUrl)),
      year: int.tryParse(value('_seriesYear', '$year')) ?? year,
      rating: double.tryParse(value('_seriesRating', '$rating')) ?? rating,
      views: views,
      isSeries: true,
      raw: rootRaw,
    );
